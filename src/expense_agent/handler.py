...
async def create_expense(request: Request):
    ...
    # Categorize via Bedrock (or local fallback)
    category = categorize_expense(entry_in.merchant, entry_in.amount)
...
async def list_expenses(request: Request):
    ...
    return JSONResponse(
        status_code=200,
        content=[json.loads(e.model_dump_json()) for e in entries],
    )
